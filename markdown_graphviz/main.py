...
import re
import markdown, markdown.preprocessors
import subprocess
import base64
...
class InlineGraphvizPreprocessor(markdown.preprocessors.Preprocessor):
    ...
    def run(self, lines):
        ...
        while 1:
            m = (
                BLOCK_RE_CURLY_BRACKET.search(text)
                if BLOCK_RE_CURLY_BRACKET.search(text)
                else BLOCK_RE_GRAVE_ACCENT.search(text)
            )
            if m:
                ...
                try:
                    ...
                    err = str(e) + " : " + str(args)
                    return (
                        "<pre>Error : " + err + "</pre>" "<pre>" + content + "</pre>"
                    ).split("\n")
